...
/**
  Config mac address to gmac

  @param MacAddress 	A point to mac address to config
...
/**
	Read mac address form gmac register
...
/**
  GMAC initialization, include the initializaion of GMAC DMA

  @param EmacDriver 	A point to GMAC dirver structure
  @param MacBaseAddress GMAC register base address	 

  @retval EFI_SUCCESS 	GMAC DXE initialization is seccessful
**/
EFI_STATUS
EFIAPI
...
/**
	GMAC DMA initialization

	@param EmacDriver	  A point to GMAC dirver structure
	@param MacBaseAddress GMAC register base address   

	@retval EFI_SUCCESS   GMAC DMA initialization is seccessful
  ...
  )
{
  ...
  //set timeout to prevent halt   by liyu 20191030
  //
  while(ResetCheckTimes){
	  if(DW_EMAC_DMAGRP_BUS_MODE_SWR_GET (MmioRead32 (MacBaseAddress + DW_EMAC_DMAGRP_BUS_MODE_OFST))){
		  gBS->Stall(100*1000);
		  ResetCheckTimes--;
		  if(ResetCheckTimes == 0){
			DEBUG((EFI_D_INFO,"gmac %0x bus mode softreset timeout!please check the ethernet cable state!!!!\n",MacBaseAddress));
...
/**
	Transmission descriptor initialization

	@param EmacDriver	  A point to GMAC dirver structure
	@param MacBaseAddress GMAC register base address   

	@retval EFI_SUCCESS   Transmission descriptor initialization is seccessful
**/
EFI_STATUS
EFIAPI
EmacSetupTxdesc (
...
/**
	...
	@retval EFI_SUCCESS   receive descriptor initialization is seccessful
**/
EFI_STATUS
EFIAPI
EmacSetupRxdesc (
...
/**
	Config GMAC register to start transmission
...
/**
	...
	@param ReceiveFilterSetting 	The config of receive filter 
	@param Reset 					Set to TRUE to reset the contents of the multicast
									receive filters on the network interface to their
									default values.
	@param NumMfilter 				Number of multicast HW MAC addresses in the new MCastFilter
	...
	@param Mfilter					A pointer to a list of new multicast receive filter HW
	...
	@retval EFI_SUCCESS   			GMAC receive filters config suceess
...
/**
	Generate the ether CRC

	@param Mac 				MAC Address
	@param AddrLen			the bytes length of MAC Address 	 

	@retval (~Remainder)	the result of generation
...
/**
	...
	@param value 			A value to bit reverse	 

	@retval 				the result of bit reverse
...
/**
	Config GMAC to stop transmission and receive 
	step:
	1.Stop DMA transmission
	2.Flush Transmit FIFO
	3.Stop transmitters
	4.Stop DMA RX
...
/**
	Config GMAC register to start DMA 
...
/**
	Get the status fo GMAC DMA

    @param IrqStat		  DMA status
	@param MacBaseAddress GMAC register base address 
**/
VOID
EFIAPI
...
/**
	Get the status fo GMAC

	@param Statistic	  GMAC status
	@param MacBaseAddress GMAC register base address 
**/
VOID
EFIAPI
...
/**
	Config the speed and duplex of GMAC
